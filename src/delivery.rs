            ...
            match delivery_state {
                ...
                State::Pending => {
                    ...
                    if gate.safe {
                        ...
                        // Gate blocked - refresh liveness so we don't go stale
                        // while waiting (DB status is still "listening" until
                        // the message is delivered and hooks fire). This path
                        // can hold for a long time behind an approval prompt,
                        // so it publishes wake state too. Supersedes the bare
                        // update_heartbeat the tracker block used to call.
                        refresh_liveness(db, &current_name);
